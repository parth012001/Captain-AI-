...
app.get('/emails/fetch', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    // Parse and save emails with user context
    let savedCount = 0;
    for (const email of emails) {
      try {
        ...
        if (!exists) {
          await emailModel.saveEmail(parsedEmail, userId);
          savedCount++;
        }
      } catch (error) {
...
if (require.main === module) {
  ...
  if (process.argv[2] === 'test') {
    ...
    app.post('/calendar/check-availability', async (req, res) => {
      try {
        ...
        console.log(`🎯 Checking availability: ${start} to ${end}`);
...
initializeServices().then(() => {
    ...
    app.post('/test-smart-filtering', async (req, res) => {
      try {
        ...
        const filterResult = await shouldGenerateResponseForEmail(mockEmail);
      ...
      try {
        ...
        for (const emailData of emailsToProcess) {
          try {
            ...
            const shouldGenerateResponse = await shouldGenerateResponseForEmail(parsedEmail);
    ...
    async function shouldGenerateResponseForEmail(email: any): Promise<{generate: boolean, reason: string}> {
      ...
      try {
        ...
        if (classification === 'newsletter') {
          return { generate: false, reason: 'AI classified as newsletter/promotional content' };
        } else {
          return { generate: true, reason: 'AI classified as personal/business communication' };
        }
      } catch (error) {
