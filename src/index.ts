...
async function initializeServices() {
  logger.info('🚀 Initializing Chief AI Email Assistant', {
  ...
  try {
    ...
    if (state) {
      try {
        ...
        console.log(`🎯 OAuth callback with intent: ${intent}`);
      } catch (error) {
        console.log('⚠️ Could not parse state parameter, continuing without intent');
      }
    }
    ...
    try {
      ...
      if (gmailAddress && tokens.refresh_token) {
        // 🔍 INTENT VALIDATION - Check user existence vs intent
        ...
        console.log(`💾 Tokens saved to database for 24/7 operation. User ID: ${userId}`);
        ...
        try {
          ...
          console.error(`⚠️ Failed to set up automatic webhook for ${gmailAddress}:`, webhookError);
    ...
    for (const email of emails) {
      try {
        ...
        if (!exists) {
          ...
          // Process new emails for meeting detection
          if (emailDbId) {
            try {
              const meetingResult = await meetingPipelineService.processEmailForMeetings(
                parsedEmail, 
                userId, 
                emailDbId
              );
              meetingProcessingResults.push(meetingResult);
            } catch (meetingError) {
              console.error(`❌ [MEETING PIPELINE] Error processing email ${parsedEmail.id}:`, meetingError);
            }
          }
        }
      } catch (error) {
...
app.get('/promotional-emails', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    console.error('❌ Error retrieving promotional emails:', error);
...
app.post('/ai/categorize-emails', authMiddleware.authenticate, async (req, res) => {
  try {
    console.log('🏷️ Categorizing recent emails...');
...
app.post('/ai/generate-drafts', authMiddleware.authenticate, async (req, res) => {
  try {
    console.log('✨ Generating drafts for recent emails...');
...
// 🚀 Phase 1: Auto-Generated Draft Management API Endpoints
...
app.put('/auto-drafts/:id', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    // 🧠 NEW: Trigger Learning Analysis (NON-BLOCKING)
    if (contentChanged) {
      // Run learning analysis asynchronously to not block the response
      setImmediate(async () => {
        try {
          console.log(`🧠 Triggering learning analysis for draft ${draftId}...`);
          ...
          console.log(`✏️  Edited content length: ${editedContent.length} chars`);
          ...
          console.error('⚠️ Learning analysis failed (edit still succeeded):', learningError);
          console.error('   This is non-critical - user edit was saved successfully');
        }
      });
    } else {
      console.log(`ℹ️ No content changes detected for draft ${draftId} - skipping learning analysis`);
    }
...
app.post('/auto-drafts/:id/send', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    console.log(`📤 Sending draft ${draftId} via Gmail API...`);
    ...
    if (draftWithEmail.context_used && draftWithEmail.context_used.meetingRequest) {
      try {
        ...
        console.log(`📅 Creating meeting confirmation for draft ${draftId}...`);
        ...
        console.log(`✅ Meeting confirmation created: ${meetingConfirmationId}`);
        
      } catch (confirmationError) {
        console.warn('⚠️ Failed to create meeting confirmation:', confirmationError);
      }
    }

    console.log(`✅ Draft ${draftId} sent successfully via Gmail API`);
    console.log(`📧 Gmail Message ID: ${sendResult.messageId}`);
    ...
    try {
      const draftId = parseInt(req.params.id);
      await autoGeneratedDraftModel.updateDraftStatus(draftId, 'sent');
      console.log(`⚠️ Draft ${draftId} marked as sent despite Gmail API error`);
    } catch (updateError) {
...
export async function testPhase1() {
  console.log('\n🧪 PHASE 1 TEST: Email Reading & Parsing\n');
  
  try {
    await initializeServices();
    
    console.log('📊 Current email stats:');
    ...
    recentEmails.forEach((email, index) => {
      console.log(`\n${index + 1}. ${email.is_read ? '✅' : '🔴'} ${email.subject}`);
  ...
  if (process.argv[2] === 'test') {
    ...
    app.post('/admin/apply-phase23-schema', async (req, res) => {
      try {
        ...
        console.log('🔧 Applying Phase 2.3 schema...');
        ...
        if (fs.existsSync(schemaPath)) {
          const schema = fs.readFileSync(schemaPath, 'utf8');
          await pool.query(schema);
          console.log('✅ Phase 2.3 schema applied successfully');
        }
        ...
        console.error('❌ Phase 2.3 schema application failed:', error);
        res.status(500).json({ error: error instanceof Error ? error.message : 'Unknown error' });
      }
    });
    ...
    app.post('/admin/fix-context-column', async (req, res) => {
      try {
        ...
        console.log('🔧 Adding missing context_used column...');
        
        const schemaPath = path.join(__dirname, '../scripts/database/add_context_used_column.sql');
        if (fs.existsSync(schemaPath)) {
          const schema = fs.readFileSync(schemaPath, 'utf8');
          await pool.query(schema);
          console.log('✅ Context_used column added successfully');
        }
    ...
    app.post('/admin/apply-phase3-calendar-schema', async (req, res) => {
      try {
        ...
        console.log('📅 Applying Phase 3 Calendar Intelligence schema...');
        ...
        console.error('❌ Error applying Phase 3 Calendar schema:', error);
        res.status(500).json({ 
    ...
    // 📅 Phase 3: Calendar Intelligence endpoints
    ...
    app.post('/calendar/set-tokens', async (req, res) => {
      try {
        ...
        console.log('✅ Calendar OAuth tokens set successfully');
    ...
    app.get('/calendar/events', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        console.log(`📅 Fetching calendar events for user ${userId} from ${start} to ${end}...`);
    ...
    // 🤖 Phase 3.2: Meeting Request Detection endpoints
    ...
    app.post('/meetings/scan-emails', async (req, res) => {
      try {
        ...
        console.log(`🔍 Scanning recent emails for meeting requests (limit: ${limit})...`);
        ...
        console.error('❌ Error scanning emails for meeting requests:', error);
    ...
    app.patch('/meetings/requests/:id', async (req, res) => {
      try {
        ...
        console.log(`📝 Updating meeting request ${id} status to ${status}...`);
    ...
    app.get('/meetings/health', async (req, res) => {
      try {
        console.log('🏥 Running meeting detection health check...');
        ...
        console.error('❌ Meeting detection health check failed:', error);
    ...
    app.get('/meetings/confirmations', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        console.log(`📋 Fetching pending meeting confirmations for user: ${userId}`);
    ...
    app.post('/meetings/confirmations/:id/confirm', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        console.log(`📅 Confirming meeting: ${id} for user: ${userId}`);
    ...
    app.post('/meetings/confirmations/:id/cancel', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        console.log(`❌ Cancelling meeting confirmation: ${id} for user: ${userId}`);
    ...
    app.get('/meetings/confirmations/:id/suggestions', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        console.log(`📅 Getting alternative time suggestions for meeting: ${id}`);
    ...
    app.post('/auto-scheduling/process-meeting', async (req, res) => {
      try {
        ...
        console.log(`🤖 Processing meeting request ${meetingRequestId} for auto-scheduling...`);
    ...
    app.post('/auto-scheduling/suggest-times', async (req, res) => {
      try {
        ...
        console.log(`⏰ Generating time suggestions for meeting request ${meetingRequestId}...`);
    ...
    app.post('/auto-scheduling/create-hold', async (req, res) => {
      try {
        ...
        console.log(`🔒 Creating calendar hold for meeting ${meetingRequestId}...`);
    ...
    app.post('/auto-scheduling/confirm', async (req, res) => {
      try {
        ...
        console.log(`✅ Confirming scheduling for meeting ${meetingRequestId}...`);
    ...
    app.get('/auto-scheduling/workflows', async (req, res) => {
      try {
        ...
        console.log(`📋 Fetching scheduling workflows (status: ${status || 'all'})...`);
    ...
    app.get('/auto-scheduling/holds', async (req, res) => {
      try {
        ...
        console.log(`🔒 Fetching calendar holds (status: ${status || 'all'})...`);
    ...
    app.get('/auto-scheduling/health', async (req, res) => {
      try {
        console.log('🏥 Running auto-scheduling system health check...');
    ...
    app.post('/ai/generate-drafts-with-context', async (req, res) => {
      try {
        console.log('✨ Generating context-aware drafts...');
        ...
        for (const email of emails) {
          try {
            ...
            console.error(`❌ Error generating draft for email ${email.id}:`, error);
          }
        }
        ...
        console.error('❌ Error generating context-aware drafts:', error);
...
initializeServices().then(() => {
    ...
    // 🧪 Test endpoint to manually trigger webhook processing (for testing)
    ...
    // 🎯 Test endpoint to specifically test smart filtering logic
    app.post('/test-smart-filtering', authMiddleware.authenticate, async (req, res) => {
      try {
        const userId = getUserId(req);
        console.log('🎯 Testing smart filtering with provided email data...');
        ...
        console.log('📧 Testing email:', mockEmail);
        
        // Test the smart filtering logic
        const filterResult = await shouldGenerateResponseForEmail(mockEmail, userId);
        
        console.log(`🤖 Filter Result: ${filterResult.generate ? '✅ PROCESS' : '❌ SKIP'} - ${filterResult.reason}`);
        ...
        console.error('❌ Error in smart filtering test:', error);
        res.status(500).json({ error: 'Smart filtering test failed' });
      }
    });

    // 🔍 Database verification endpoint for testing draft management
    ...
    // 🧪 Test endpoint to manually create a draft for testing draft management
    app.post('/test-create-draft', async (_req, res) => {
      try {
        console.log('🧪 Manually creating test draft for draft management testing...');
        ...
        console.log(`✅ Test draft created with ID: ${draftId}`);
    ...
    // 🚀 MULTI-USER WEBHOOK PROCESSING - Processes emails for the specific user in notification
      ...
      try {
        ...
        for (const emailData of emailsToProcess) {
          try {
            ...
            // Step 5: Generate smart response using Phase 2.3 API
            const responseRequest = {
              emailId: emailId.toString(),
              recipientEmail: parsedEmail.from,
              originalSubject: parsedEmail.subject,
              originalBody: parsedEmail.body,
              responseType: 'reply' as const,
              userId: userId // ✅ Add userId for user profile integration
            };

            console.log(`🤖 Generating smart response for email ID ${emailId}...`);
            const smartResponse = await responseService.generateSmartResponse(responseRequest);

            // Step 6: Store auto-generated draft
            ...
            console.log(`✅ Auto-generated draft created with ID ${autoDraftId} in ${processingTime}ms`);
            console.log(`📝 Subject: "${smartResponse.subject}"`);
            console.log(`🎯 Tone: ${smartResponse.tone}, Urgency: ${smartResponse.urgencyLevel}`);

            // 🚀 NEW: Process email through meeting pipeline (Phase 1 & 2)
            try {
              console.log(`🔍 [WEBHOOK] Processing email ${parsedEmail.id} through meeting pipeline...`);
              const meetingResult = await meetingPipelineService.processEmailForMeetings(
                parsedEmail,
                userId,
              ...
              if (meetingResult.isMeetingRequest) {
                console.log(`📅 [WEBHOOK] Meeting detected! Type: ${meetingResult.meetingRequest?.meetingType}, Confidence: ${meetingResult.confidence}%`);
                if (meetingResult.response) {
                  console.log(`🤖 [WEBHOOK] Response generated: ${meetingResult.response.actionTaken} (confidence: ${meetingResult.response.confidenceScore}%)`);
                }
              } else {
                console.log(`📧 [WEBHOOK] No meeting detected for email ${parsedEmail.id}`);
              }
            } catch (meetingError) {
              console.error(`❌ [WEBHOOK] Meeting pipeline error for email ${parsedEmail.id}:`, meetingError);
              // Don't fail the entire webhook processing if meeting pipeline fails
            }
    ...
    async function shouldGenerateResponseForEmail(email: any, userId: string): Promise<{generate: boolean, reason: string, classification?: string}> {
      ...
      if (fromEmail.includes('parthahir012001@gmail.com')) {
        console.log(`🛑 [EMAIL FILTER] Skipping AI-generated email to prevent infinite loop: ${subject}`);
        return { generate: false, reason: 'AI-generated email skipped to prevent infinite loop' };
      }
    ...
    async function _storeProcessingResult(result: any) {
      try {
        ...
        console.log(`💾 Storing processing result: ${result.type}`);
      } catch (error) {
      ...
      try {
        ...
        for (const userTokens of activeUsers) {
          try {
            ...
            console.error(`❌ Failed to setup webhook for ${userTokens.gmailAddress}:`, userError);
            results.push({
              user: userTokens.gmailAddress,
              success: false,
        ...
        console.error('❌ Error setting up Gmail webhooks for all users:', error);
    ...
    app.get('/webhooks/status', async (_req, res) => {
      try {
        ...
        console.error('❌ Error getting webhook heartbeat:', error);
    ...
    console.log('\n🔍 System Monitoring:');
        ...
        console.log('\n📡 Gmail Webhooks (Real-time Email):');
