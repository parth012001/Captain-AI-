...
initializeServices().then(() => {
    ...
    // 🚀 PHASE 3: Intelligent Router Test Endpoint
    app.post('/test-intelligent-router', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        for (const email of emailsToTest) {
          console.log(`🧠 [ROUTER TEST] Testing email: "${email.subject}"`);
          ...
          console.log(`✅ [ROUTER TEST] Result: ${routingResult.routingDecision.route.toUpperCase()} (${routingResult.routingDecision.confidence}%)`);
        }
        
        // Generate statistics
        const stats = intelligentEmailRouter.getRoutingStats(routingResults);
        
        console.log(`📊 [ROUTER TEST] Stats:`, stats);
        ...
        console.error('❌ Intelligent router test failed:', error);
        res.status(500).json({ 
    ...
    // 🚀 PHASE 3: Intelligent Router Health Check Endpoint
    app.get('/health/intelligent-router', async (_req, res) => {
      try {
        console.log('🏥 [ROUTER HEALTH] Running intelligent router health check...');
        ...
        console.log(`🏥 [ROUTER HEALTH] Status: ${healthCheck.status}`, healthCheck);
    ...
    async function shouldGenerateResponseForEmail(email: any, userId: string): Promise<{generate: boolean, reason: string, classification?: string}> {
      ...
      // 🛡️ INFINITE LOOP PREVENTION - TEMPORARILY DISABLED FOR TESTING
      ...
      for (const pattern of aiSenderPatterns) {
        if (fromEmail.includes(pattern.toLowerCase())) {
          console.log(`🛑 [INFINITE LOOP PREVENTION] Blocked AI sender: ${pattern}`);
      ...
      for (const pattern of aiSubjectPatterns) {
        if (subject.toLowerCase().includes(pattern.toLowerCase())) {
          console.log(`🛑 [INFINITE LOOP PREVENTION] Blocked AI subject: ${pattern}`);
      ...
      for (const pattern of aiBodyPatterns) {
        if (body.toLowerCase().includes(pattern.toLowerCase())) {
          console.log(`🛑 [INFINITE LOOP PREVENTION] Blocked AI body content: ${pattern}`);
